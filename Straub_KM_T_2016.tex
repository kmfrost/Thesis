...
\begin{itemize}
\item Techniques such as random forests work well for small, low-dimensional data.  However, if the size of the dataset were greatly increased to billions of people a more sophisticated model would be necessary for accurate classification.  This is required to accurately learn the complicated structure inherent to such large real-world datasets.
...
\item Imagine a representative sample of volunteers from the dataset self-identifies themselves.  This produces a set of labeled training data that can be used for backpropagation.  In conjunction with greedy pre-training, backpropagation has been shown to improve both the optimization and generalization of a DBN.
