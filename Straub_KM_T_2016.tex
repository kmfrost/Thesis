...
A deep belief network (DBN) is one model used to solve these types a problems. 
...
Typically, the layers of nodes have undirected connections.
...
A DBN is shown in Figure~\ref{fig:DBN}.

\begin{figure}[t]
    \centering
        \includegraphics[width=.7\columnwidth,trim={1mm 2mm 1mm 3.5mm},clip]{dbn}
        \vspace{-7pt}
        \caption[Deep Belief Network]{A deep belief network is a graphical network constructed by layering Restricted Boltzman machines on top of Sigmoid Belief Networks.}
        \label{fig:DBN}
\end{figure}

For the nodes in the rest of these layers, there exists a probability of activation, $p(s_i=1)$, which is represented by the nonlinear sigmoid function applied to a weighted input from the layer above.
Specifically,
\begin{equation}
...
Thereore, the DBN with $l$ hidden layers is able to model the join distribution between inputs $x$ and those hidden layers:
\begin{equation}
P(x, h^1, ..., h^l) = \left(\prod_{k=0}^{l-2}P(h^k|h^{k+1} \right) P(h^{l-1},h^l)
\end{equation}

Therefore $P(h^{l-1},h^l)$ represents the joint distribution of the top two layers, which form the RBM.
...
\begin{enumerate}
\item Train an RBM on the input, $x$, as the bottom layer of the network.
...
\item Add a linear classifier
\item Fine-tune all the parameters of the deep network using a supervised training method
\end{enumerate}
...
The purpose of this research is to label online reviews as positive, negative, or neutral.
...
Overall, this improves the training learning efficiency of the algorithm.
This method can classify sentiments more accurately and train more quickly than previous semi-supervised algorithms.
It is a classifier such as this that could be used to powerfully learn with extremely large email datasets, particularly those without hashed text.
