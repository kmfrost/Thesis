...
\begin{figure}[t]
    \centering
    \includegraphics[width=\columnwidth,trim={0mm 0mm 0mm 5mm},clip]{JetEdit}
    \vspace{-17pt}
...
&= \arg\min_a H(\text{Class}|{Attribute})
\end{align}
...
\begin{figure}[t]
	\centering
	\includegraphics[width=0.7\columnwidth,trim={0mm 0mm 0mm 0mm},clip]{RandForest}
	\vspace{-15pt}
...
The emails in the dataset were randomly assigned to one of the following: 35\% for training, 30\% for cross-validation, and 35\% for testing.
The training set was used to train the dataset by building the random trees.
The cross-validation set was used in lieu of a test set to optimize the parameters of the random forest.
...
Different sectioning percentages were tried for the training and cross-validation sets, but 35\% for training and 30\% for cross-validation produced the best cross-validation accuracy.
This is likely because it is nearly an even split between the two sets.
From cross-validation analysis, the optimum number of trees in the random forest was determined to be 750.
Another test found that each tree should subsample of 7 random features for maximum cross-validation accuracy.
...
\begin{figure}[t]
	\centering
	\includegraphics[width=\columnwidth,trim={0mm 0mm 0mm 0mm},clip]{generated_org_chart}
	\vspace{-7pt}
...
This organic organization chart reveals the underlying relationships within the center that differ from the official organization chart.
