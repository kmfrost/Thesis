...
To compute a symmetry metric, 
For this metric, the diagonal of the matrix is ignored.

The scale is normalized to measure symmetry such that a perfectly antisymmetric matrix would evaluate to negative one and a perfectly symmetric matrix would evaluate to one.
...
If instead the trees were constructed by subsampling emails for all employees and calculating the features at that level, a different prior would have been instilled in the trees.
This new prior would be influenced by the distribution of emails by class, represented by the blue bar in Figure~\ref{fig:db_bar}, which does not reflect the prior distribution of people into classes.
